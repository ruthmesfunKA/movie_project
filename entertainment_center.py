...
class Imdb():
	...
	def duration(self): # getting the duration from the data
		tree = self.page_tree()
		duration = tree.xpath("//time[@itemprop='duration']/text()")[1]
		return duration
	
	def movie_storyline(self): # storyline 
		tree = self.page_tree()
		movie_storyline = tree.xpath("//div[@class='summary_text']/text()")[0]
		return movie_storyline

	def poster_image(self): #  poster image
		tree = self.page_tree()
		poster_image = tree.xpath("//img/@src")[2]
		return poster_image
	# def title(self.page):
	# 	title 
